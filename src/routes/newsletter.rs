...
use actix_web::{
    http::header::HeaderMap, http::header::HeaderValue, web, HttpResponse, ResponseError,
};
use anyhow::Context;
...
fn basic_authentication(headers: &HeaderMap) -> Result<Credentials, anyhow::Error> {
    ...
    let decoded_credentials = String::from_utf8(decoded_bytes)
        .context("The decoded credential string is not avalid UTF8")?;
...
async fn validate_credentials(
    ...
    .await
    .context("Failed to spawn blockign task")
    .map_err(PublishError::UnexpectedError)??;
    ...
    fn error_response(&self) -> HttpResponse {
        match self {
            ...
            PublishError::AuthError(_) => {
                ...
                let header = HeaderValue::from_str(r#"Basic realm="publish""#).unwrap();
                response
                    .headers_mut()
